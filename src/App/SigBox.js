...
class SigBox extends Component {
  ...
  captureCanvas = () => {
    let { canvas } = this;
    let url = this.refs.post_uri.value;
    if (!url) return alert('Please enter a valid POST URL...');

    let body = { id: null, data: canvas.toDataURL() };

    axios.post(url, body)
      .then(resp => console.log('Local entries: ', resp.data))
      .catch(err => console.log(err));
  }

  requestSignature = () => {
    let url = this.refs.get_uri.value;
    if (!url) return alert('Please enter a valid GET URL...');

    axios.get(url)
      .then(resp => this.writeSignature(resp.data.data))
      .catch(err => console.log(err))
  }
    ...
    return (
      <section className="sig_box_main">
        ...
        <canvas id='canvas'
                ...
                height="200"
                width="450"
                style={{ "border":"2px solid #222222", "background":"#FFFFFF" }} >
        </canvas><br />

        <button onClick={this.clearCanvas}>Clear</button>
        <button onClick={() => this.writeSignature(dummySig())}>Print Demo</button><br /><br /><br />

        <p>Enter URI to POST current image to (default '/api/signature/new'):</p><br />
